...
python3 - "$TweeFile" "$PassagesFile" << 'PYEOF'
import re, sys
...
with open(output_path, 'w') as out:
    for p in passages[1:]:
        ...
        tags = match.group(2).strip() if match and len(match.groups()) > 1 else ''
        body_lines = []
        for line in lines[1:]:
            if line.startswith(':: '):
                break
        ...
        out.write(f'<tw-passagedata name="{name}"{tag_attr}>{body}</tw-passagedata>\n')
...
PLACEHOLDER = '___PASSAGES_PLACEHOLDER___'
html = '''<!DOCTYPE html>
...
</html>'''.replace(PLACEHOLDER, passages_content)
